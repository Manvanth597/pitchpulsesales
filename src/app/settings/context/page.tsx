...
export default function SettingsContextPage() {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem("pitchContext")
    if (stored) {
      try {
        ...
        form.reset({
          ...
          coreValueProposition: parsed.coreValueProposition || "",
        })
        if (parsed.additionalContext) {
          setAdditionalContext(parsed.additionalContext)
        }
      } catch (e) {
        console.error("Failed to parse stored context")
      }
    }
  }, [form])
        ...
        <Card className="p-6 sm:p-8 border-border/60 shadow-sm">
          <div className="mb-8">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Update the details about your offering to improve generated sales narratives.
            </p>
          </div>
